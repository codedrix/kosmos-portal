...
<main class="container mx-auto px-4 py-8 max-w-2xl">
	<div class="mb-6">
		<a href="/upload/details" class="text-sm text-indigo-600 hover:text-indigo-700"
			>&larr; Back to Details</a
		>
	</div>
	...
	{#if publishSuccess && publishedUrl}
		<div class="bg-green-50 border border-green-200 rounded-xl p-8 text-center mb-8">
			...
			<div
				class="bg-white border border-green-300 rounded-lg px-4 py-3 inline-flex items-center gap-3"
			>
		...
		<div class="bg-white rounded-lg border border-gray-200 divide-y divide-gray-200">
							...
							{#if data.details.capabilities.length > 0}
								<div class="flex flex-wrap justify-end gap-1">
									{#each data.details.capabilities as cap}
										<span
											class="inline-block px-2 py-0.5 text-xs bg-indigo-50 text-indigo-700 rounded-full"
										>
											{formatCap(cap)}
										</span>
									{/each}
			...
			<div class="p-6">
				...
				<div class="bg-green-50 border border-green-200 rounded-lg p-4 flex items-start gap-3">
					...
					<div>
						<p class="text-sm font-medium text-green-800">26 validation rules checked</p>
			...
			<div class="p-6">
				...
				<div
					class="w-full aspect-video bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 text-sm"
				>
					1280 x 720 preview
				</div>
			</div>
